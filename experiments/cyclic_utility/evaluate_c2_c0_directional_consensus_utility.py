"""Evaluate preregistered C2-C0 behind a strict pre-GT directional seal.

Formal execution is exposed for a later separately authorized phase.  The
implementation phase exercises only pure unit-test paths and temporary seals.
"""
...
STAGE = c2.STAGE
DATASET = "Caltech-6V"
FROZEN_C2_B0_MANIFESTS = (
    "experiment_freeze/c2_b0_implementation_preformal_20260905/"
    "c2_b0_source_sha256.txt",
    "experiment_freeze/c2_b0_multiseed_fail_closed_20260908/"
    "source_sha256.txt",
)
PRE_GT_NPZ_NAME = "c2_c0_directional_pre_gt.npz"
DIRECTIONAL_SEAL_NAME = "c2_c0_directional_seal.json"
PRE_GT_ARRAY_NAMES = (
    "sample_ids",
    "unlabeled_ids",
    "labeled_sample_ids",
...
PER_SEED_OUTPUT_FILES = (
    PRE_GT_NPZ_NAME,
    DIRECTIONAL_SEAL_NAME,
    "c2_c0_metrics_by_direction.json",
    "c2_c0_confidence_strata_audit.json",
    "c2_c0_seed_summary.json",
    "c2_c0_audit.json",
)
...
def verify_frozen_c2_b0_sources():
    """Verify both required C2-B0 manifests before any C2-C0 execution."""
    manifests = OrderedDict()
    for name in FROZEN_C2_B0_MANIFESTS:
        audit = b0_evaluator.verify_sha256_manifest(name)
        _require(
            audit.get("all_entries_exact_match") is True,
            "C2-C0 frozen C2-B0 SHA256 verification failed",
        )
...
def default_paths(seed, output_dir=None):
    ...
    source = b0_evaluator.default_paths(active_seed)
    target = (
        REPOSITORY_ROOT
        / "outputs/cyclic_utility"
        / ("c2_c0_directional_consensus_utility_seed" + str(active_seed))
...
def load_c0_inputs_before_gt(seed, c0_artifact_path, c0_seal_path):
    arrays, provenance = b0_evaluator.load_c0_inputs_before_gt(
    ...
    _require(
        ...
        "C2-C0 pre-GT C0 loader crossed an oracle boundary",
    )
    return arrays, provenance


def load_fixed_sparse_labels(split_dir):
    split = b0_evaluator.load_fixed_sparse_labels(split_dir)
    validated = c2.validate_fixed_sparse_split(
        split["labeled_sample_ids"], split["unlabeled_ids"],
...
def build_pre_gt_bundle(
    ...
    """Construct C2-C0 arrays without accepting any truth-bearing input."""
    active_seed = c2.validate_seed(seed)
    _require(
        ...
        "C2-C0 pre-GT bundle received leaked provenance",
    )
    _require(
        frozen_boundary.get("all_frozen_C2_B0_hashes_pass") is True,
        "C2-C0 cannot proceed without frozen C2-B0 hash verification",
    )
    outputs = c2.build_directional_consensus_outputs(
    ...
    arrays = OrderedDict((
        ...
        ("d_L", outputs["d_L"]),
        ("D", outputs["D"]),
        ("U_tilde_dir", outputs["U_tilde_dir"]),
        ("z_L_shuffle", outputs["z_L_shuffle"]),
        ("e_L_shuffle", outputs["e_L_shuffle"]),
        ("d_L_shuffle", outputs["d_L_shuffle"]),
        ("D_shuffle", outputs["D_shuffle"]),
        ("U_tilde_dir_shuffle", outputs["U_tilde_dir_shuffle"]),
    ))
    _require(tuple(arrays) == PRE_GT_ARRAY_NAMES, "C2-C0 pre-GT key whitelist mismatch")
    return {
        "seed": active_seed,
        "arrays": arrays,
        "outputs": outputs,
...
def persist_directional_bundle(bundle, output_dir):
    """Write, flush, fsync, reload, and verify every logical array hash."""
    output_root = _resolve(output_dir)
    _require(not output_root.exists(), "refusing to overwrite C2-C0 seed output")
    arrays = bundle["arrays"]
    _require(tuple(arrays) == PRE_GT_ARRAY_NAMES, "C2-C0 persistence schema mismatch")
    ...
    with np.load(npz_path, allow_pickle=False) as archive:
        _require(tuple(archive.files) == PRE_GT_ARRAY_NAMES, "C2-C0 sealed NPZ field mismatch")
        for name in PRE_GT_ARRAY_NAMES:
            ...
            _require(
                ...
                "C2-C0 sealed array reload/hash mismatch: " + name,
            )
    ...
    seal = {
        ...
        "frozen_C2_B0_source_verification": bundle["frozen_boundary"],
        "true_target_hash": array_records["labeled_targets"]["logical_sha256"],
        "shuffle_target_hash": array_records["shuffled_labeled_targets"][
            "logical_sha256"
        ],
        "true_directional_hashes": {
            name: array_records[name]["logical_sha256"]
            for name in ("z_L", "e_L", "d_L", "D", "U_tilde_dir")
        },
        "shuffle_directional_hashes": {
            name: array_records[name]["logical_sha256"]
            for name in (
                "z_L_shuffle", "e_L_shuffle", "d_L_shuffle", "D_shuffle",
                "U_tilde_dir_shuffle",
            )
        },
        "NPZ_saved_fsynced_reloaded_hash_verified": True,
        "GT_loaded_before_directional_seal": False,
        "c0_audit_loaded_before_directional_seal": False,
        "Bridge_correctness_loaded_before_directional_seal": False,
        ...
        "C_conf_used_for_directional_propagation": False,
        "residual_magnitude_propagated": False,
        "shuffle_recomputed_from_targets": True,
        "shuffle_only_final_D": False,
        "parameter_free_utility_update": True,
    }
    _require(
        seal["true_target_hash"] != seal["shuffle_target_hash"],
        "C2-C0 true/shuffle target hashes unexpectedly match",
    )
    seal_path = output_root / DIRECTIONAL_SEAL_NAME
    write_json_durable(seal_path, seal)
    _fsync_directory(output_root)
    _require(read_json(seal_path) == seal, "C2-C0 directional seal durable reload mismatch")
    return {
        ...
        "durable_directional_seal_verified": True,
    }


def load_full_ground_truth_after_directional_seal(path):
    return b0_evaluator.load_full_ground_truth_after_residual_seal(path)


def load_c0_audit_after_directional_seal(c0_audit_path, c0_prediction_seal):
    return b0_evaluator.load_c0_audit_after_residual_seal(
        c0_audit_path, c0_prediction_seal
    )


def load_native_global_cluster_after_directional_seal(
    c0_artifact_path, c0_prediction_seal
):
    return b0_evaluator.c2_a0_evaluator.load_native_global_cluster_after_calibration_seal(
        c0_artifact_path, c0_prediction_seal
    )


def build_bridge_correctness_after_directional_seal(arrays, c0_audit, full_GT):
    return b0_evaluator.build_bridge_correctness_after_residual_seal(
        arrays, c0_audit, full_GT
    )


def evaluate_after_directional_seal(bundle, persistence, paths):
    """Load all correctness-bearing objects only after durable verification."""
    _require(
        persistence.get("durable_directional_seal_verified") is True
        and read_json(persistence["seal_path"]) == persistence["seal"],
        "C2-C0 durable directional seal is not verified",
    )
    full_GT, full_GT_audit = load_full_ground_truth_after_directional_seal(
        paths.full_gt_path
    )
    c0_audit, c0_audit_provenance = load_c0_audit_after_directional_seal(
        paths.c0_audit_path, bundle["C0_provenance"]["C0_prediction_seal"]
    )
    native = load_native_global_cluster_after_directional_seal(
        paths.c0_artifact_path, bundle["C0_provenance"]["C0_prediction_seal"]
    )
    correct, mapping_audit = build_bridge_correctness_after_directional_seal(
    ...
    _require(
        ...
        "C2-C0 post-seal evaluation split mismatch",
    )
    ...
    diagnostic = c2.analyze_directional_sign_agreement(
        arrays["D"][unlabeled], arrays["D_shuffle"][unlabeled],
        e_gt, unlabeled,
    )
    calibration = c2.analyze_calibration_directions(
        correct[unlabeled], arrays["U_cycle"][unlabeled],
        arrays["U_tilde_dir"][unlabeled],
        arrays["U_tilde_dir_shuffle"][unlabeled],
        arrays["C_conf"][unlabeled], unlabeled,
    )
    summary = c2.build_seed_summary(bundle["seed"], diagnostic, calibration)
    return {
        "directional_diagnostic": diagnostic,
        "calibration": calibration,
        "summary": summary,
        "full_GT_audit": {**full_GT_audit, "loaded_after_directional_seal": True},
        "c0_audit_provenance": {
            **c0_audit_provenance, "loaded_after_directional_seal": True
        },
...
def save_postseal_results(bundle, persistence, evaluation):
    ...
    metrics = {
        ...
        "seed": bundle["seed"],
        "evaluation_sample_count": c2.UNLABELED_EVAL_COUNT,
        "directional_diagnostic": evaluation["directional_diagnostic"],
        "calibration": calibration,
    }
    audit = {
        "stage": STAGE,
        "seed": bundle["seed"],
        "C2_C0_AUDIT_PASS": True,
        "directional_seal": persistence["seal"],
        "directional_seal_file_sha256": persistence["seal_file_sha256"],
        "durable_directional_seal_verified_before_GT": True,
        "GT_loaded_before_directional_seal": False,
        "GT_loaded_after_directional_seal": True,
        "c0_audit_loaded_before_directional_seal": False,
        "c0_audit_loaded_after_directional_seal": True,
        "Bridge_correctness_loaded_before_directional_seal": False,
        "Bridge_correctness_built_after_directional_seal": True,
        "corruption_mask_loaded": False,
        "training_performed": False,
        "model_forward_called": False,
        "optimizer_created": False,
        ...
        "residual_magnitude_propagated": False,
        "DirectionalDiagnostic_used_to_construct_U_tilde_dir": False,
    ...
    records = OrderedDict((
        ("c2_c0_metrics_by_direction.json", metrics),
        ("c2_c0_confidence_strata_audit.json", strata),
        ("c2_c0_seed_summary.json", evaluation["summary"]),
        ("c2_c0_audit.json", audit),
    ))
    ...
    _require(
        ...
        "C2-C0 post-seal result durable reload mismatch",
    )
    return records
...
def run_evaluation(seed, output_dir=None, full_gt_path=None):
    ...
    frozen_boundary = verify_frozen_c2_b0_sources()
    active_seed = c2.validate_seed(seed)
    ...
    persistence = persist_directional_bundle(bundle, paths.output_dir)
    evaluation = evaluate_after_directional_seal(bundle, persistence, paths)
    records = save_postseal_results(bundle, persistence, evaluation)
    print("C2_C0_SEED_PASS=" + str(evaluation["summary"]["C2_C0_SEED_PASS"]))
