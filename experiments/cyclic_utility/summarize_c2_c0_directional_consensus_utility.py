"""Summarize exactly the three preregistered C2-C0 seed bundles."""
...
MULTISEED_OUTPUT_FILES = (
    "c2_c0_multiseed_summary.json",
    "c2_c0_multiseed_decision.json",
    "c2_c0_multiseed_audit.json",
)
...
def default_multiseed_dir():
    return (
        REPOSITORY_ROOT
        / "outputs/cyclic_utility/c2_c0_directional_consensus_utility_multiseed"
    )
...
def load_seed_bundle(seed, seed_dir):
    ...
    _require(root.is_dir(), "C2-C0 seed directory is missing")
    paths = {name: root / name for name in REQUIRED_SEED_FILES}
    _require(
        all(path.is_file() for path in paths.values()),
        "C2-C0 seed output schema is incomplete",
    )
    seal = evaluate.read_json(paths[evaluate.DIRECTIONAL_SEAL_NAME])
    metrics = evaluate.read_json(paths["c2_c0_metrics_by_direction.json"])
    summary = evaluate.read_json(paths["c2_c0_seed_summary.json"])
    audit = evaluate.read_json(paths["c2_c0_audit.json"])
    strata = evaluate.read_json(paths["c2_c0_confidence_strata_audit.json"])
    _require(
        ...
        "C2-C0 seed identity mismatch",
    )
    _require(
        evaluate.file_sha256(paths[evaluate.PRE_GT_NPZ_NAME])
        == seal.get("pre_gt_npz_file_sha256")
        and audit.get("directional_seal") == seal
        and audit.get("directional_seal_file_sha256")
        == evaluate.file_sha256(paths[evaluate.DIRECTIONAL_SEAL_NAME])
        and seal.get("NPZ_saved_fsynced_reloaded_hash_verified") is True
        and audit.get("durable_directional_seal_verified_before_GT") is True,
        "C2-C0 seed durable-seal boundary mismatch",
    )
    false_boundaries = (
        ...
        "C_conf_used_for_directional_propagation",
        "residual_magnitude_propagated",
        "DirectionalDiagnostic_used_to_construct_U_tilde_dir",
        "DirectionalDiagnostic_used_in_gate",
    ...
    _require(
        ...
        and audit.get("GT_loaded_after_directional_seal") is True
        and audit.get("c0_audit_loaded_after_directional_seal") is True
        and audit.get("Bridge_correctness_built_after_directional_seal") is True
        and audit.get("evaluation_only_unlabeled") is True
        and audit.get("evaluation_sample_count") == c2.UNLABELED_EVAL_COUNT,
        "C2-C0 seed leakage/evaluation boundary mismatch",
    )
    recomputed = c2.build_seed_summary(
        active_seed,
        metrics["directional_diagnostic"],
        metrics["calibration"],
    )
    _require(summary == recomputed, "C2-C0 seed summary differs from gate helper")
    return {
        "seed": active_seed,
...
def summarize(seed20_dir, seed30_dir, seed50_dir, output_dir=None):
    frozen_boundary = evaluate.verify_frozen_c2_b0_sources()
    directories = OrderedDict(((20, seed20_dir), (30, seed30_dir), (50, seed50_dir)))
    _require(tuple(directories) == c2.SEEDS, "C2-C0 multi-seed set/order mismatch")
    bundles = OrderedDict(
    ...
    _require(not output_root.exists(), "refusing to overwrite C2-C0 multi-seed output")
    audit = {
        "stage": STAGE,
        "seeds": list(c2.SEEDS),
        ...
        "frozen_C2_B0_source_verification": frozen_boundary,
        "seed_directories": {
    ...
    records = OrderedDict((
        ("c2_c0_multiseed_summary.json", aggregate["summary"]),
        ("c2_c0_multiseed_decision.json", aggregate["decision"]),
        ("c2_c0_multiseed_audit.json", audit),
    ))
    _require(tuple(records) == MULTISEED_OUTPUT_FILES, "C2-C0 multi-seed schema mismatch")
    ...
    _require(
        ...
        "C2-C0 multi-seed durable reload mismatch",
    )
