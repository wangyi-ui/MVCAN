"""Seal-first post-GT evaluator for one P0-A2 diagnostic arm."""
...
def evaluate(*, arm, run_dir, dataset_path):
    ...
    root = Path(run_dir)
    _require(
        root.resolve() == protocol.default_output_dir(active_arm).resolve(),
        "run directory does not match the frozen P0-A2 arm path",
    )
    ...
    _require(
        ...
        and manifest.get("initialization_source_condition")
        == protocol.INITIALIZATION_SOURCE_CONDITION,
        "run manifest arm, initialization, or GT-firewall mismatch",
    )
...
def main(argv=None):
    ...
    result = evaluate(
        arm=args.arm, run_dir=args.run_dir, dataset_path=args.dataset_path
    )
    print(json.dumps({
        "arm": args.arm,
        "acc": result.acc,
