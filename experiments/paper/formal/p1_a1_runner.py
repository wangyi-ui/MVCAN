...
def run_formal(run):
    ...
    try:
        ...
        if run.arm == "BASE":
            provenance = wiring.base_provenance(feature=paths["features"], feature_audit=paths["feature_audit"], split=paths["split"], split_audit=paths["split_audit"], initialization=initialization, output=paths["output"])
            print("[P1-A1] BASE native-only provenance verified", flush=True)
            print("[P1-A1] pre-GT training started", flush=True)
            sealed = base_runtime.run_base_pre_gt(runtime, provenance)
        elif run.arm == "OURS_TRUE_U":
            ...
            raise RuntimeError("FORMAL_ABLATION_RUNTIME_NOT_WIRED")
